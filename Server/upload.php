...
if (($_FILES["uploaded"]["size"] < 20000000))
  ...
  if ($_FILES["uploaded"]["error"] > 0)
	...
	$videodirtemp = "/home/cs5248-15/team06/public_html/upl/" .$folderName. "/temp";
	$videopathtemp = "$videodirtemp/$videoname";
	$videodir = "/home/cs5248-15/team06/public_html/upl/" .$folderName. "/high";
	...
	$cmdvar = "/usr/local/bin/ffmpeg -i ".$videopathtemp." -codec:v libx264 -profile:v high -preset slow -b:v 500k -maxrate 500k -bufsize 1000k -vf scale=-1:720 -threads 0 -codec:a libfdk_aac -b:a 128k -y ".$videodir."/output1.mp4";
	$cmdvar1 = "/usr/local/bin/ffmpeg -i ".$videopathtemp." -codec:v libx264 -profile:v high -preset slow -b:v 500k -maxrate 500k -bufsize 1000k -vf scale=-1:480 -threads 0 -codec:a libfdk_aac -b:a 128k -y ".$videodir."/output2.mp4";
	$cmdvar2 = "/usr/local/bin/ffmpeg -i ".$videopathtemp." -codec:v libx264 -profile:v high -preset slow -b:v 500k -maxrate 500k -bufsize 1000k -vf scale=-1:240 -threads 0 -codec:a libfdk_aac -b:a 128k -y ".$videodir."/output3.mp4";
	...
	if( $xml = file_get_contents( 'info.xml') ) {
	echo "sunnnnnnnnnns";
